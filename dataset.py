...
class NiftiImageGenerator(Dataset):
    def __init__(self, imagefolder, input_size, depth_size, transform=None):
        ...
        self.inputfiles = glob(os.path.join(imagefolder, '*.jpg'))
    ...
    def read_image(self, file_path):
        ...
        # Масштабируем пиксели изображения в диапазоне от 0 до 1
        img /= 255.0
        # Преобразуем изображение с помощью scaler (если необходимо)
...
class NiftiImageGenerator2d(Dataset):
    ...
    def read_image(self, file_path):
        ...
        # Преобразуем изображение в массив numpy
        img = np.array(img).astype(np.float32)
        # Если изображение цветное (3D: HxWxC), берем средний слой по третьей оси
        if len(img.shape) == 3:
            img = img[:, :, img.shape[2] // 2]
        # Масштабируем значения пикселей в диапазоне от 0 до 1
        img /= 255.0
        # Преобразуем изображение с помощью scaler (если необходимо)
        img = self.scaler.fit_transform(img.reshape(-1, 1)).reshape(img.shape)
        return img
        # img = nib.load(file_path).get_fdata()
        # img = img[:,:, img.shape[2] // 2]
        # img = self.scaler.fit_transform(img.reshape(-1, img.shape[-1])).reshape(img.shape) # 0 -> 1 scale
...
class NiftiPairImageGenerator2d(Dataset):
    ...
    def read_image(self, file_path, pass_scaler=False):
        ...
        # Преобразуем изображение в массив numpy
        img = np.array(img).astype(np.float32)
        # Если изображение цветное (3D: HxWxC), выбираем центральный канал
        if len(img.shape) == 3:
            img = img[:, :, img.shape[2] // 2]
        # Масштабируем значения пикселей в диапазоне от 0 до 1
        img /= 255.0
        # Применяем scaler, если pass_scaler=False
        if not pass_scaler:
            img = self.scaler.fit_transform(img.reshape(-1, 1)).reshape(img.shape)
        return img
        # img = nib.load(file_path).get_fdata()
